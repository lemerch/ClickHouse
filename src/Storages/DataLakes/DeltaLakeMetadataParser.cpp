...
struct DeltaLakeMetadataParser<Configuration, MetadataReadHelper>::Impl
{
        ...
        ContextPtr context)
    {
        ...
        while (!buf->eof())
        {
            ...
            if (object->has("add"))
            {
                ...
                auto & current_parition_columns = file_partition_columns[filename];
                if (current_parition_columns.empty())
                {
                    ...
                    for (const auto & name : partition_values->getNames())
                    {
                        ...
                        current_parition_columns.emplace_back(*name_and_type, field);
    ...
    Field getFieldValue(const String & value, DataTypePtr data_type)
    {
        ...
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Unsupported DeltaLake type for {}", data_type->getColumnType());
    }
